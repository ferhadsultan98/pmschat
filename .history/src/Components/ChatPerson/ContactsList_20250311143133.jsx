...
const ContactsList = ({
  ...
  const handleAddContact = async (contact) => {
    const userId = sessionStorage.getItem("userId");
    const senderContactRef = ref(
      database,
      `chats/${userId}/contacts/${contact.id}`
    );
    const receiverContactRef = ref(
      database,
      `chats/${contact.id}/contacts/${userId}`
    );
    ...
    try {
      ...
      await set(receiverContactRef, {
        ...
        avatar:
          sessionStorage.getItem("avatar") || "https://via.placeholder.com/40",
        lastMessage: "No messages yet",
        timestamp: Date.now(),
      });
