...
import { Search, Plus, More вертик, LogOut } from "react-feather";
import {
  database,
  ref,
  onValue,
...
const ContactsList = ({ onContactSelect, currentUser, isOpen }) => {
          ...
          (usersSnapshot) => {
            ...
            const contactsWithLastMessage = Array.from(chatUserIds)
              .map((userId) => {
                ...
                const chatWithUser = userChats.find(
                  ([_, chat]) => chat.participants && chat.participants[userId]
                );
  ...
  const handleSearch = (query) => {
    ...
    setShowNoDataMessage(false); // Arama değiştiğinde mesajı sıfırla
  };
  ...
  useEffect(() => {
    if (localSearchQuery && filteredContacts.length === 0) {
  ...
  }, [localSearchQuery, filteredContacts]);
    ...
    <>
      <div className={`contactsList ${isOpen ? "isOpen" : ""}`}>
        ...
        <div className="contactsContainer">
          ...
          ) : localSearchQuery && showNoDataMessage ? (
