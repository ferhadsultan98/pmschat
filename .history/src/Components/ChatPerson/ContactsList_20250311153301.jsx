...
const ContactsList = ({ selectedContact, onContactSelect, searchQuery, onSearch }) => {
  ...
  // Fetch contacts on mount and listen for changes
  useEffect(() => {
  ...
  // Close the more menu when clicked outside
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className={`contactsList ${isOpen ? 'isOpen' : ''}`}>
        ...
        <div className="contactsContainer">
          {filteredContacts.map((contact) => (
            <div
              key={contact.id}
              className={`contactItem ${selectedContact?.id === contact.id ? 'contactItemSelected' : ''}`}
      ...
      {isModalOpen && (
        <ContactsModal onClose={() => setIsModalOpen(false)} onContactSelect={handleAddContact} />
      )}
    </>
  );
};
