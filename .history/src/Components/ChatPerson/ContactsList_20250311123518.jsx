...
const ContactsList = ({ selectedContact, onContactSelect, onAddButtonClick, onSearch, searchQuery }) => {
  ...
  useEffect(() => {
    const userId = sessionStorage.getItem("userId"); // Oturum açmış kullanıcının ID’sini al
    if (!userId) {
      navigate("/login"); // Kullanıcı oturum açmadıysa login sayfasına yönlendir
      return;
    }

    const chatsRef = ref(database, `chats/${userId}/contacts`); // Kullanıcıya özel yazışmalar
      ...
      (snapshot) => {
        ...
        if (data) {
          const contactsArray = Object.entries(data).map(([id, contact]) => ({
            id,
            name: contact.name || "Bilinmeyen Kullanıcı",
          ...
          {filteredContacts.map((contact) => (
            <div
              ...
              className={`contactItem ${
                selectedContact && selectedContact.id === contact.id ? "contactItemSelected" : ""
              }`}
            ...
            >
              ...
              <div className="contactTimestamp">{formatTimestamp(contact.timestamp)}</div>
            </div>
          ))}
        </div>
      </div>
