...
const ChatWindow = ({ contact, currentUser, isOpen, toggleWindow }) => {
  ...
  const [closingDropdownId, setClosingDropdownId] = useState(null); // Yeni state
  ...
  const toggleDropdown = (messageId) => {
    if (openDropdownId === messageId) {
      setClosingDropdownId(messageId); // Kapanma animasyonu için
      setTimeout(() => {
        setOpenDropdownId(null);
        setClosingDropdownId(null); // Temizle
      }, 200); // Animasyon süresiyle eşleşiyor (0.2s)
    } else {
