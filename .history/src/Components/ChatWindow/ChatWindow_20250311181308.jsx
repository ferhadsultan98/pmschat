...
import { getAuth } from "firebase/auth"; // Firebase Authentication'dan kullanıcıyı almak için
...
const ChatWindow = ({ contact }) => {
  ...
  const auth = getAuth(); // Firebase Authentication'dan oturum açmış kullanıcıyı al
  const currentUser = auth.currentUser; // Mevcut kullanıcı

  // Sohbet odası ID'sini oluştur (alfabetik sırayla, tutarlılık için)
  ...
  const handleSendMessage = (e) => {
    ...
    const newMessage = {
      senderId: currentUser.uid, // Gönderenin ID'si
      text: message.trim(),
    ...
    push(messagesRef, newMessage)
      .then(() => {
        // Sohbet odasını güncelle: son mesaj ve zaman damgası
        update(chatRoomRef, {
