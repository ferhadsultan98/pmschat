...
const ChatWindow = ({ contact, currentUser, isOpen, toggleWindow }) => {
  ...
  // Fetch messages and scroll to bottom when contact changes
  useEffect(() => {
    ...
    // Scroll to bottom immediately when contact changes
    scrollToBottom();

    return () => unsubscribe();
  }, [contact?.id, currentUser?.userId]); // Dependency on contact.id ensures this runs when contact changes
  ...
  }, [messages]); // Dependency on messages ensures this updates when messages change
