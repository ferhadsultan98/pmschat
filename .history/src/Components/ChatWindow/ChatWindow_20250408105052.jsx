...
const ChatWindow = ({ contact, currentUser, isOpen, toggleWindow }) => {
  ...
  useEffect(() => {
    // If we don't have a contact with chat ID or current user, return early
    if (!contact?.chatId || !currentUser?.userId) return;
    
    // Use the provided chatId from the contact object
    ...
    return () => unsubscribe();
  }, [contact?.chatId, currentUser?.userId]);
  ...
  const handleSendMessage = (e) => {
    e.preventDefault();
    if (!message.trim() || !contact?.chatId || !currentUser?.userId) return;
    
    // Use the chatId from the contact object
    const messagesRef = ref(database, `chats/${contact.chatId}/messages`);
    ...
    const newMessage = {
      ...
      text: message.trim(),
      timestamp: Date.now(), // Use actual timestamp for immediate display
    };
    
    push(messagesRef, newMessage)
      .then(() => {
        // Update last message in the chat document
        const chatRef = ref(database, `chats/${contact.chatId}`);
        update(chatRef, {
