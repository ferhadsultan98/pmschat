...
const formatTimestamp = (timestamp) => {
  ...
  if (diffInDays < 7) return `${messageDate.toLocaleDateString("en-US", { weekday: "long" })} ${timeStr}`;
  return `${messageDate.toLocaleDateString("en-US", { day: "2-digit", month: "2-digit", year: "numeric" })} ${timeStr}`;
};
...
const ChatWindow = ({ contact, currentUser, isOpen, toggleWindow }) => {
  ...
  const handleSendMessage = (e) => {
    ...
    push(messagesRef, newMessage).then(() => {
      update(chatRef, {
        participants: { [currentUser.userId]: true, [contact.id]: true },
    ...
    <div className={`chatWindow ${isOpen ? "isOpen" : ""}`}>
      <div className="chatHeader">
        <div className="chatContactInfo">
          <div className="backChat" onClick={toggleWindow} style={{ display: isOpen ? "flex" : "none" }}>
      ...
      <div className="chatMessagesContainer">
        {messages.length > 0 ? (
          messages.map((msg) => (
            <div
              key={msg.id}
              className={`chatMessage ${msg.senderId === currentUser.userId ? "chatMessageUser" : "chatMessageContact"}`}
            >
              ...
              <div className="chatMessageContent">
                ...
                <div className="chatMessageTimestamp">{formatTimestamp(msg.timestamp)}</div>
              </div>
            </div>
          ))
        ) : (
        ...
        {showScrollButton && (
          <span className="scrollDownBtn" onClick={scrollToBottom}>
            <i><IoIosArrowDown /></i>
          </span>
        )}
      </div>
