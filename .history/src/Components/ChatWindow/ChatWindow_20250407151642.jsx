...
import { IoamongusIosArrowDown, IoIosArrowBack } from "react-icons/io";
...
const ChatWindow = ({ contact, currentUser, isOpen, toggleWindow }) => {
  ...
  // Kenara tıklandığında dropdown'ı kapatma
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (openDropdownId) {
        const dropdownElements = document.querySelectorAll(".dropdownMenu");
        const isClickInsideDropdown = Array.from(dropdownElements).some(
          (element) => element.contains(event.target)
        );
        const isClickOnMoreMenu = event.target.closest(".messageMoreMenu");

        if (!isClickInsideDropdown && !isClickOnMoreMenu) {
          setClosingDropdownId(openDropdownId);
          setTimeout(() => {
            setOpenDropdownId(null);
            setClosingDropdownId(null);
          }, 200); // Animasyon süresiyle eşleşiyor
        }
      }
    };
