...
const ChatWindow = ({ contact, currentUser, isOpen, toggleWindow }) => {
  ...
  const handleCopy = (text, event) => {
    ...
    // Tıklama dropdown dışında gerçekleştiyse kapat
    if (event && event.target) {
      const dropdownElement = document.getElementById('dropdown-id'); // Dropdown'ın ID'sini buraya ekleyin
      if (dropdownElement && !dropdownElement.contains(event.target)) {
        setOpenDropdownId(null);
      }
    } else {
      // Event yoksa doğrudan kapat
      setOpenDropdownId(null);
    }
  };
