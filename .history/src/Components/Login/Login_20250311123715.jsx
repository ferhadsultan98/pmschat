...
const Login = () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (userFound) {
          ...
          sessionStorage.setItem("userId", userId); // Rastgele ID’yi sakla
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      const userId = uuidv4(); // Rastgele ID oluştur
      const usersRef = ref(database, `users/${userId}`); // Rastgele ID ile kaydet
      const newUser = {
        fullName,
      ...
      await set(usersRef, newUser); // push yerine set kullanıyoruz çünkü ID belirli
