...
const Login = () => {
  ...
  // Login işlemi başarılıysa, user bilgilerini localStorage'a kaydediyor
  const login = (user) => {
    localStorage.setItem('user', JSON.stringify(user));
  };

  // Eğer kullanıcı giriş yapmışsa yönlendirme yapıyoruz.
  useEffect(() => {
    if (localStorage.getItem('user')) {
      navigate("/chat");  // Eğer giriş yapılmışsa chat sayfasına yönlendir
    }
  }, [navigate]);
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (userFound) {
          ...
          login(user);  // Kullanıcıyı localStorage'a kaydediyoruz

          toast.success("Login successful! Redirecting...", {
            position: "top-right",
            autoClose: 2000,
            onClose: () => navigate("/chat"),  // Yönlendirme işlemi
          });
        } else {
