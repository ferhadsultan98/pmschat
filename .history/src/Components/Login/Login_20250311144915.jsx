...
import { isAuthenticated } from "../../Utils/auth"; // Import to verify auth state
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (userFound) {
          ...
          // Kullanıcı objesini oluştur ve login fonksiyonunu çağır
          const user = { id: userId, ...userData };
          login(user); // 'user' anahtarını sessionStorage'a kaydeder
  
          // Mevcut sessionStorage ayarlarını isteğe bağlı olarak tutabilirsiniz
          sessionStorage.setItem("userId", userId);
          sessionStorage.setItem("username", userData.username);
          sessionStorage.setItem("fullName", userData.fullName);
  
          console.log("sessionStorage after login:", sessionStorage.getItem("userId"));
  
          toast.success("Login successful! Redirecting...", {
            position: "top-right",
            autoClose: 2000,
            onClose: () => {
              console.log("isAuthenticated before navigation:", isAuthenticated());
