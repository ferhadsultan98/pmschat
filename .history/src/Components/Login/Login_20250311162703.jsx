...
const Login = () => {
  ...
  // Login işlemi başarılıysa, user bilgilerini ve token'ı localStorage'a kaydediyor
  const login = (user) => {
    const token = uuidv4(); // Generate a unique token
    const authData = {
      ...user,
      token, // Add token to the auth data
    };
    localStorage.setItem("auth", JSON.stringify(authData)); // Store user data with token
  };

  // Eğer kullanıcı giriş yapmışsa (token varsa) yönlendirme yapıyoruz
  useEffect(() => {
    const authData = JSON.parse(localStorage.getItem("auth"));
    if (authData && authData.token) {
      navigate("/chat"); // Token varsa chat sayfasına yönlendir
    }
  }, [navigate]);
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (userFound) {
          ...
          login(user); // Kullanıcıyı ve token'ı localStorage'a kaydet
        ...
        <div className={`card ${isFlipped ? "flipped" : ""}`}>
          ...
          <div className="cardBack">
            <form onSubmit={handleRegisterSubmit} className="loginRegisterForm">
              ...
              {imageFile && <img src={URL.createObjectURL(imageFile)} alt="Preview" style={{ maxWidth: "30px" }} />}
